...
class SAM2(BaseModule):
    def __init__(
        ...
        hydra_overrides = [
            ...
            "++model._target_=projects.salma.models.predictor.SAM2VideoPredictor",
        ]
    ...
    def preprocess_image(
        self, image: torch.Tensor, dtype=torch.float32
    ) -> torch.Tensor:
        image = image / 255.0

        img_mean = torch.tensor(self.img_mean, dtype=dtype, device=image.device)[
            :, None, None
        ]
        img_std = torch.tensor(self.img_std, dtype=dtype, device=image.device)[
            :, None, None
        ]
        image -= img_mean
        image /= img_std
