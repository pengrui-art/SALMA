...
from projects.salma.utils.losses import masked_avg_pool, tmc_loss, boundary_loss
...
from projects.salma.models.preprocess.image_resize import DirectResize
...
class VideoLLaVASAMModel(LisaModel):
    def __init__(
        ...
        if hasattr(self.mllm, "_post_init"):
            self.mllm._post_init(
                fast_pool_size=self.fast_pool_size, fast_pool=self.fast_pool
            )
        else:
        ...
        # If cross-modal attention exists inside grounding encoder, unfreeze it for training
        if (
            hasattr(self.grounding_encoder, "cma")
            and self.grounding_encoder.cma is not None
        ):
            self.grounding_encoder.cma.requires_grad_(True)

        if self.mllm.use_llm_lora:
            if self.arch_type == "intern_vl":
                self.mllm.model.language_model.base_model.model.get_input_embeddings().requires_grad_(
                    True
                )
                self.mllm.model.language_model.base_model.model.get_output_embeddings().requires_grad_(
                    True
                )
            elif self.arch_type == "qwen":
                self.mllm.model.model.base_model.model.get_input_embeddings().requires_grad_(
                    True
                )
        ...
        if use_fast_supervision:
            self.loss_exists = BUILDER.build(
                dict(
        ...
        self._warned_tmc_skip = False

        if fast_pool:
            self.fast_token_idx = self.tokenizer(
                "<FAST_IMG_CONTEXT>", add_special_tokens=False
            ).input_ids[0]
        else:
    ...
    def set_aux_loss_warmup_scale(self, scale: float) -> None:
        """Hook entry point to smoothly ramp auxiliary losses."""
        scale = float(max(scale, 0.0))
        self._aux_loss_warmup_scale = float(min(scale, 1.0))
    ...
    def _merge_lora(self):
        ...
        try:
            self.mllm.model.language_model = (
                self.mllm.model.language_model.merge_and_unload()
            )
        except:
            print("Skip language model, no LoRA in it !!!")
        try:
            self.mllm.model.vision_model = (
                self.mllm.model.vision_model.merge_and_unload()
            )
        except:
    ...
    def state_dict(self, *args, **kwargs):
        ...
        to_return.update(
            {k: v for k, v in state_dict.items() if "grounding_encoder.cma." in k}
        )

        # Step 5. others (fcs)
        to_return.update(
            {k: v for k, v in state_dict.items() if "text_hidden_fcs." in k}
        )
        to_return.update(
            {k: v for k, v in state_dict.items() if "text_exist_fcs." in k}
        )
        to_return.update(
            {
                k: v
    ...
    def check_obj_number(
        self, pred_embeddings_list_video, gt_masks_video, fix_number=5
    ):
        ...
        for pred_mebeds, gt_masks in zip(pred_embeddings_list_video, gt_masks_video):
            ...
            if len(pred_mebeds) != fix_number:
                if len(pred_mebeds) > fix_number:
                    ...
                    pred_mebeds = torch.cat([pred_mebeds] * n_repeat, dim=0)[
                        :fix_number
                    ]
    ...
    def forward(self, data, data_samples=None, mode="loss"):
        ...
        if self.fast_pool:
            output = self.mllm(
                data, data_samples, mode, fast_token_idx=self.fast_token_idx
            )
        else:
        ...
        if self.use_fast_supervision and fast_exists is not None:
            ...
            exists_logit = self.text_exist_fcs(
                fast_tokens[self.fast_pool_size**2 - 1 :: self.fast_pool_size**2]
            )
        ...
        pred_masks_per_frame = pred_masks  # (num_frames, num_objs, H, W)
        resized_size = pred_masks_per_frame.shape[-2:]
        gt_masks = []
        for gt_mask in gt_masks_video:
            resized = F.interpolate(
                gt_mask.unsqueeze(0), size=resized_size, mode="nearest"
            ).squeeze(0)
            resized = resized.to(
                device=pred_masks_per_frame.device, dtype=pred_masks_per_frame.dtype
            )
            gt_masks.append(resized)
        gt_masks_tensor = torch.stack(gt_masks, dim=0)
        gt_masks = gt_masks_tensor.flatten(0, 1)
        pred_masks = pred_masks_per_frame.flatten(0, 1)
        ...
        if seg_valid:
            ...
            if self.loss_tmc_weight > 0.0 and need_lowres_feat:
                try:
                    if lowres_feat is None:
                        raise ValueError("lowres_feat is required for TMC loss")
                    if pred_masks_per_frame.dim() != 4:
                        raise ValueError("Expected pred_masks with shape (F,O,H,W)")
                    nf, nobj, Hm, Wm = pred_masks_per_frame.shape
                    N_total = nf * nobj
                    if lowres_feat.dim() != 4 or lowres_feat.size(0) != N_total:
                        raise ValueError("lowres_feat shape mismatch")
                    masks_source = (
                        gt_masks_tensor
                        if self.tmc_use_gt_mask
                        else pred_masks_per_frame
                    )
                    masks_flat = masks_source.reshape(N_total, Hm, Wm)
                    v_feat = (
                        lowres_feat.detach() if self.tmc_detach_visual else lowres_feat
                    )
                    vis_roi = masked_avg_pool(v_feat, masks_flat)  # (N_total, C)
                    text_flat = language_embeddings.squeeze(1)
                    if text_flat.dim() != 2:
                        raise ValueError("language embeddings must be 2D after squeeze")
                    min_n = min(text_flat.size(0), vis_roi.size(0))
                    if min_n == 0:
                        raise ValueError("Empty features for TMC loss")
                    vis_roi = vis_roi[:min_n]
                    text_flat = text_flat[:min_n]
                    c_loss = tmc_loss(
                        vis_roi, text_flat, temperature=self.tmc_temperature
                    )
                except Exception as exc:
                    if not self._warned_tmc_skip:
                        logging.getLogger(__name__).warning(
                            "Skipping TMC loss this run due to mismatch: %s", exc
                        )
                        self._warned_tmc_skip = True
                    aux_losses.setdefault(
                        "loss_tmc",
                        pred_masks.new_zeros(()),
                    )
                else:
                    aux_losses["loss_tmc"] = c_loss * self.loss_tmc_weight * aux_scale

        if self.loss_tmc_weight > 0.0 and "loss_tmc" not in aux_losses:
            aux_losses["loss_tmc"] = pred_masks.new_zeros(())
        if self.loss_boundary_weight > 0.0 and "loss_boundary" not in aux_losses:
            aux_losses["loss_boundary"] = pred_masks.new_zeros(())
    ...
    def sample_points(self, mask_pred, gt_masks):
        ...
        with torch.no_grad():
            ...
            mask_point_targets = point_sample(gt_masks.float(), points_coords).squeeze(
                1
            )
        ...
        return mask_point_preds.to(mask_pred.dtype), mask_point_targets.to(
            mask_pred.dtype
        )
    ...
    def genetate_video_pred_embeddings(self, pred_embeddings_list, frames_per_batch):
        if len(pred_embeddings_list) == len(frames_per_batch):
            ...
            success = False
            print(
                "len(pred_embeddings_list):{} is not equal to len(frames_per_batch):{} !!!".format(
                    len(pred_embeddings_list), len(frames_per_batch)
                )
            )
        pred_embeddings_list_video = []
        for pred_embedding_batch, frame_nums in zip(
            pred_embeddings_list, frames_per_batch
        ):
    ...
    def process_video_gt_masks(self, gt_masks, frames_per_batch):
        ...
        for gt_masks_batch, frames_num in zip(gt_masks, frames_per_batch):
            ...
            gt_masks_batch = gt_masks_batch.reshape(N // frames_num, frames_num, H, W)
    ...
    def preparing_for_generation(self, metainfo, **kwargs):
        ...
        if self.phi3:
            self.mllm.model.language_model.prepare_inputs_for_generation = MethodType(
                prepare_inputs_for_generation, self.mllm.model.language_model
            )
        return
    ...
    def predict_video(self, pixel_values, text_prompts, **kwargs):
        ...
        for input_ids in _input_ids:
            ...
            if len(seg_hidden_states) == 0:
                ...
                pred_masks.append(
                    torch.zeros(
                        (g_pixel_values.shape[0], ori_h, ori_w), dtype=torch.int
                    )
                )
                continue
            ...
            if len(pixel_values) < 5:
                pred_mask = self.grounding_encoder.language_embd_inference(
            ...
            # supervision
            if (
                self.use_fast_supervision
                and (input_ids == self.fast_token_idx).sum() > 0
            ):
                fast_flag = input_ids.squeeze(0) == self.fast_token_idx
                len_out = generate_output.sequences[0][:-1].shape[0]
                fast_tokens = last_hidden_states[:-len_out][fast_flag].to(
                    dtype=torch.float32
                )
                exists_logit = self.text_exist_fcs(
                    fast_tokens[self.fast_pool_size**2 - 1 :: self.fast_pool_size**2]
        ...
        ret_dict = {
            ...
            "prediction_masks": [
                mask_to_rle(_item.cpu().numpy()) for _item in pred_masks
            ],
        }
