...
from projects.salma.models.internvl import InternVL_Slowfast

from projects.salma.models import (
    VideoLLaVASAMModel,
    SAM2TrainRunner,
    VideoLLaVASAMModel_zero3,
)
from projects.salma.datasets import (
    VideoReVOSDataset,
    VideoMeVISDataset,
...
from projects.salma.datasets import VideoChatUniViDataset
from projects.salma.datasets import (
    RefCOCOgGCGDataset,
    OpenPsgGCGDataset,
    FlickrGCGDataset,
...
from projects.salma.datasets import LLaVADataset
from projects.salma.datasets import ReferSegmDataset
from projects.salma.models.preprocess.image_resize import DirectResize
from projects.salma.utils.rank_logging import PerRankLogHook
from projects.salma.utils.resume_progress import ResumeProgressHook

# Warmup hooks for CMA / auxiliary losses
from projects.salma.utils.cma_warmup_hook import CMAWarmupHook
from projects.salma.utils.aux_warmup_hook import AuxLossWarmupHook
...
# Let MMEngine import our custom hook so it registers into HOOKS registry
custom_imports = dict(
    imports=[
        "projects.salma.utils.cma_warmup_hook",
        "projects.salma.utils.aux_warmup_hook",
        "projects.salma.utils",
        "projects.salma",
    ],
...
#######################################################################
# Model
path = "/data1/pengrui/Model/OpenGVLab/InternVL2_5-4B/"
pretrained_pth = None
...
model = dict(
    ...
    grounding_encoder=dict(
        ...
        # Cross-Modal Attention (CMA) options: enable and sensible defaults for 4B training
        enable_cma=True,
        cma_num_heads=8,
        ...
        # Mask bias and phrase routing (phase3 options)
        enable_mask_bias=True,
        mask_bias_mode="gate",
        mask_bias_tau=1.0,
        mask_bias_strength=1.0,
        mask_source="pred",
        cma_use_phrase_routing=True,
        cma_phrase_chunk_size=0,
        # Multi-scale injection (disabled at start)
        cma_apply_on_highres=False,
        cma_highres_levels=(-1,),
    ),
    loss_mask=dict(
    ...
    # Phase 2 optional losses (enabled for phase3 experiments)
    loss_tmc_weight=0.1,
...
custom_hooks = [
    ...
    # Warm up auxiliary losses (TMC/boundary)
